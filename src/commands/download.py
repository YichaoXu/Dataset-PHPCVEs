...
def download(
    ...
    output_dir: Path = typer.Option(Path("downloads"), help="Directory to store downloaded code"),
    ...
    with ProgressUI(len(filtered_records), "Downloading code") as progress:
        for record in filtered_records:
            ...
            if not repo_url or not current_commit:
                progress.log_warning(f"Skipped {cve_id}: Missing repository or commit information")
            ...
            if not force and (cve_dir / "current").exists() and (cve_dir / "previous").exists():
            ...
            # Create directories
            ensure_dir(cve_dir / "current")
            ensure_dir(cve_dir / "previous")
            
            # Download current version
            progress.update(0, f"{cve_id} (current)")
            current_success = github_api.download_commit(
                owner, repo, current_commit, cve_dir / "current"
            )
            
            # Download previous version
            if previous_commit:
                progress.update(0, f"{cve_id} (previous)")
                previous_success = github_api.download_commit(
                    owner, repo, previous_commit, cve_dir / "previous"
                )
            else:
                previous_success = False
                progress.log_warning(f"No previous commit available for {cve_id}")
            
            # Update progress
            if current_success and previous_success:
                progress.log_success(f"Downloaded both versions for {cve_id}")
            elif current_success:
                progress.log_warning(f"Downloaded only current version for {cve_id}")
            else:
